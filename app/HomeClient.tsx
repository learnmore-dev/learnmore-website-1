              ...
              >
                {Array.from({ length: totalSlides }).map((_, slideIndex) => (
                  <div key={slideIndex} className="w-full flex-shrink-0 px-4">
                    <div className="flex md:grid md:grid-cols-3 gap-8 overflow-x-auto md:overflow-visible snap-x snap-mandatory scrollbar-none pb-4">
                      {featuredCourses.slice(slideIndex * 3, slideIndex * 3 + 3).map((course: any, idx: number) => (
                        <div
                          key={course.id}
                          className="w-[85vw] md:w-auto flex-shrink-0 snap-center group bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 hover:-translate-y-2"
                        >
